...
const FooterNav = ({authenticated, user}) => {
    ...
    if (user) {
      authenticatedOptions = (
        <ul className="flex flex-wrap items-center mb-2 text-sm text-gray-500 sm:mb-0 dark:text-gray-400">
            ...
            <li>
                <a href={`/cart/${user.id}`} className="mr-4 hover:underline md:mr-6">Cart</a>
            </li>
            <li>
                <a href={`/favorite/${user.id}`} className="mr-4 hover:underline md:mr-6 ">Favorites</a>
            </li>
            <li>
                <a href={`/settings/${user.id}`} className="hover:underline">Settings</a>
            </li>
        </ul>
  ...
  <div>
    ...
    <hr className="mt-2 mb-4 border-gray-200 sm:mx-auto dark:border-gray-700 lg:my-8" />
